...
const BENEFITS = [
  {
    num: '30%',   animate: true, target: 30, suffix: '%',
    title: 'Reduction in Electricity Cost',
    desc:  'Industries on our PPA model see 15–30% savings on electricity bills compared to standard grid tariffs, from day one. No capital risk to you.',
    wide:  true,
  },
  {
    num: '₹0',   animate: false,
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: '20px', padding: '2.4rem',
        transition: 'all 0.38s ease',
        transform: hov ? 'translateY(-7px)' : 'translateY(0)',
        boxShadow: hov ? '0 24px 64px rgba(249,115,22,0.13)' : '0 2px 16px rgba(0,0,0,0.05)',
    ...
    >
      ...
      <div style={{
        ...
        transformOrigin: 'left', transition: 'transform 0.38s ease',
      }} />
      ...
      <div
        id={counterId}
        style={{
          fontFamily: "'Barlow Condensed',sans-serif", fontSize: '3.4rem', fontWeight: 900,
      ...
      <div style={{ fontSize: '0.98rem', fontWeight: 700, color: '#0C1220', marginBottom: '0.45rem' }}>{card.title}</div>
      <div style={{ fontSize: '0.85rem', color: '#64748B', lineHeight: 1.7 }}>{card.desc}</div>

      {/* Decorative circle */}
      <div style={{
        position: 'absolute', bottom: '-30px', right: '-30px',
        width: '120px', height: '120px', borderRadius: '50%',
        background: 'rgba(249,115,22,0.05)', transition: 'all 0.4s',
...
export default function Benefits() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.from('.ben-card', {
        ...
        opacity: 0, y: 45, stagger: 0.1, duration: 0.7, ease: 'power3.out',
      })
      ...
      ScrollTrigger.create({
        ...
        onEnter: () => {
          gsap.to({ v: 0 }, {
            v: 30, duration: 2.4, ease: 'power2.out',
            onUpdate: function () {
  ...
  return (
    <section id="benefits" ref={sRef} style={{
      ...
      <div style={{ maxWidth: '1280px', margin: '0 auto' }}>
        ...
        {/* Header */}
        <div className="ben-header" style={{ textAlign: 'center', marginBottom: '4.5rem' }}>
          <div className="sec-label" style={{ justifyContent: 'center' }}>What You Gain</div>
          <h2 style={{
